...
<div style="display:flex;flex-direction:column;gap:24px;">
    ...
    <div class="card" style="overflow:hidden;">
        ...
        <div class="table-row" style="padding:14px 20px;border-bottom:1px solid rgba(30,53,96,.4);display:flex;align-items:flex-start;gap:12px;">
            ...
            <div style="flex:1;">
                <p style="font-size:13px;font-weight:500;color:#fff;margin-bottom:2px;">{{ $ann->title }}</p>
                <p style="font-size:12px;color:#64748b;margin:0;">{{ $ann->author->name }} · {{ $ann->created_at->diffForHumans() }}</p>
            </div>
            <span class="badge badge-{{ $ann->type === 'urgent' ? 'error' : ($ann->type === 'maintenance' ? 'warning' : 'info') }}" style="text-transform:capitalize;flex-shrink:0;">{{ $ann->type }}</span>
        </div>
        @empty
