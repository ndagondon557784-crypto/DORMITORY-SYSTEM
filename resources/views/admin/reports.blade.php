...
<div class="mb-8">
    <h1 class="text-2xl font-extrabold text-gray-800">Reports & Analytics</h1>
    <p class="text-gray-400 text-sm mt-1">Occupancy, demographics, and application statistics</p>
</div>

<!-- Summary Cards -->
<div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
    @foreach([
        ['Pending Applications',  $applicationStats['pending'],  'text-yellow-600','bg-yellow-50 border-yellow-100'],
        ['Approved Applications', $applicationStats['approved'], 'text-green-600', 'bg-green-50 border-green-100'],
        ['Rejected Applications', $applicationStats['rejected'], 'text-red-600',   'bg-red-50 border-red-100'],
    ] as [$l,$v,$tc,$bg])
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100">
        <h2 class="font-extrabold text-gray-800">Room Occupancy Report</h2>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-[#004D98] text-white">
                <tr>
                    ...
                    <th class="px-5 py-3 text-left text-xs font-bold uppercase tracking-wider">%</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-5 py-3.5 font-semibold text-gray-700">{{ $room->capacity }}</td>
                    <td class="px-5 py-3.5 font-semibold text-gray-700">{{ $room->occupied }}</td>
                    <td class="px-5 py-3.5 font-bold {{ ($room->capacity-$room->occupied)>0?'text-green-600':'text-red-500' }}">
                        {{ max(0,$room->capacity-$room->occupied) }}
                    </td>
                    ...
                    <td class="px-5 py-3.5">
                        <span class="px-2.5 py-1 rounded-full text-xs font-bold
                            {{ $room->status==='available' ? 'bg-green-100 text-green-700' :
                               ($room->status==='full'      ? 'bg-red-100 text-red-700'     : 'bg-yellow-100 text-yellow-700') }}">
                            {{ ucfirst($room->status) }}
                        </span>
                    </td>
                </tr>
