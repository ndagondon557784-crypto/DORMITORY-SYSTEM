...
<div class="max-w-3xl">
    <div class="mb-6">
        <h1 class="text-2xl font-extrabold text-gray-800">Apply for a Room</h1>
    ...
    <div class="bg-yellow-50 border border-yellow-200 rounded-2xl p-6 text-center">
        ...
        <p class="text-yellow-700 text-sm">There are currently no available rooms matching your gender. Please check back later or contact the administrator.</p>
    </div>
    @else
    <form method="POST" action="{{ route('student.apply.store') }}" class="space-y-5">
        ...
        <div class="space-y-4">
            ...
            <label class="block cursor-pointer group">
                ...
                <div class="bg-white border-2 border-gray-200 rounded-2xl p-5 transition-all
                            peer-checked:border-[#004D98] peer-checked:bg-blue-50 group-hover:border-blue-300 group-hover:shadow-md">
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex-1">
                            ...
                            <p class="text-gray-500 text-sm mb-3">{{ $room->building }} · Floor {{ $room->floor }}</p>
                            @if($room->description)
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            <label class="block text-xs font-bold text-gray-700 mb-1.5 uppercase tracking-wider">Reason for Application (optional)</label>
            <textarea name="reason" rows="3" placeholder="Tell us why you'd like this room..."
                      class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#004D98] bg-gray-50 focus:bg-white">{{ old('reason') }}</textarea>
        </div>

        <div class="flex gap-3">
            <button type="submit" class="px-8 py-3 bg-[#A50044] text-white font-extrabold rounded-xl hover:bg-[#7A003C] transition text-sm shadow-lg active:scale-[.98]">
                Submit Application
            </button>
            <a href="{{ route('dashboard') }}" class="px-8 py-3 border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 transition text-sm font-semibold">
                Cancel
            </a>
        </div>
    </form>
