...
<div class="max-w-3xl">
    <div class="mb-6 flex items-center justify-between flex-wrap gap-3">
        <div>
            <h1 class="text-2xl font-extrabold text-gray-800">My Profile</h1>
            <p class="text-gray-400 text-sm mt-1">Your student account information</p>
        </div>
        <a href="{{ route('student.profile.edit') }}"
           class="px-4 py-2 bg-[#004D98] text-white font-bold rounded-xl text-sm hover:bg-[#003a73] transition">
            ✏️ Edit Profile
        </a>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
        <div class="bg-[#004D98] px-6 py-5 text-white flex items-center gap-4">
            <div class="w-14 h-14 rounded-full flex items-center justify-center font-black text-2xl flex-shrink-0
                {{ $student && $student->gender === 'male' ? 'bg-blue-300 text-blue-900' : 'bg-pink-300 text-pink-900' }}">
                {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
            </div>
            <div>
                <h2 class="text-xl font-black">{{ auth()->user()->name }}</h2>
                <p class="text-blue-200 text-sm">
                    {{ $student->student_number ?? 'N/A' }} · {{ $student->course ?? 'N/A' }}
                </p>
            </div>
        </div>
        <div class="p-6 grid grid-cols-2 md:grid-cols-3 gap-5">
            @foreach([
                ...
                ['Year',      'Year '.($student->year_level ?? 'N/A')],
            ...
            <div>
                ...
                <p class="text-gray-800 font-semibold text-sm break-words">{{ $v }}</p>
            </div>
            @endforeach
        </div>
        @if($student && $student->address)
        <div class="px-6 pb-5">
            <p class="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1">Home Address</p>
            <p class="text-gray-700 text-sm">{{ $student->address }}</p>
        </div>
        @endif
    </div>
        ...
        <div class="flex items-center gap-4 bg-blue-50 rounded-xl p-4">
            ...
            <div class="flex-1">
                ...
                <p class="text-gray-400 text-xs mt-1">
                    ...
                    {{ $allocation->end_date?->format('M d, Y') ?? 'Open-ended' }}
                </p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100">
            <h3 class="font-extrabold text-gray-800">Allocation History</h3>
        </div>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-5 py-3.5 font-semibold text-gray-800">
                        Room {{ $h->room->room_number }}
                        <span class="text-gray-400 font-normal text-xs ml-1">{{ $h->room->building }}</span>
                    </td>
                    ...
                    <td class="px-5 py-3.5">
                        <span class="px-2.5 py-1 rounded-full text-xs font-bold
                            {{ $h->status==='active' ? 'bg-green-100 text-green-700' :
                               ($h->status==='ended'  ? 'bg-gray-100 text-gray-500'  : 'bg-red-100 text-red-600') }}">
                            {{ ucfirst($h->status) }}
                        </span>
                    </td>
                </tr>
        ...
        @else
        <div class="px-6 py-10 text-center text-gray-400 text-sm">No allocation history yet.</div>
        @endif
    </div>
</div>
