...
<div class="max-w-3xl">
    <div class="mb-6">
        <h1 class="text-2xl font-extrabold text-gray-800">My Applications</h1>
        <p class="text-gray-400 text-sm mt-1">Track all your room application requests</p>
    </div>
    ...
    <div class="bg-gradient-to-br from-[#004D98] to-[#003a73] text-white rounded-2xl p-6 mb-6 shadow-xl">
        ...
        <a href="{{ route('student.room') }}"
           class="inline-block mt-3 px-5 py-2 bg-[#EDBB00] text-[#004D98] font-bold rounded-xl text-sm hover:bg-yellow-400 transition">
            View Room Details →
        </a>
    </div>
    @endif
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-10 text-center">
        ...
        <a href="{{ route('student.apply') }}"
           class="inline-block px-6 py-3 bg-[#A50044] text-white font-bold rounded-xl hover:bg-[#7A003C] transition text-sm">
            Apply for a Room
        </a>
    </div>
    @else
            ...
            <div class="flex items-center gap-4 p-5 flex-wrap">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-gray-500 text-sm">
                        {{ $app->room->building }} · {{ ucfirst($app->room->type) }}
                        · ₱{{ number_format($app->room->price_per_month,2) }}/mo
                    </p>
                ...
                <form method="POST" action="{{ route('student.apply.cancel', $app) }}"
                    ...
                    @csrf
                    <button type="submit"
                            class="px-4 py-2 bg-red-50 text-red-600 text-xs rounded-xl hover:bg-red-100 transition font-bold whitespace-nowrap">
                        Cancel
                    </button>
                </form>
                @endif
